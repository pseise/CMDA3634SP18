...
int main(int argc, char **argv){
  ...
  float *count_h = (float*) malloc(Nre*Nim*sizeof(float));
  ...
  Mandelbrot<<<G, B>>>(Nre, Nim, cmin, cmax, count_d); 
  
  cudaMemcpy(count_h,count_d,Nre*Nim*sizeof(float),cudaMemcpyDeviceToHost);
  ...
  write_hot_png(fp, Nre, Nim, count_h, 0, 80);
  printf("done.\n");
